...
Líneas 73-80 (Lógica Matemática y Manejo del Estándar SNMP): Si ambos valores son positivos, calcula matemáticamente el porcentaje restante y lo redondea analíticamente con [Math]::Round().
...
Líneas 142-149 (Mecanismo anti-sobreescritura): Verifica la existencia física del JSON local. Si existe, lee su contenido y lo analiza con json.load(f) para transformarlo en un diccionario activo en memoria.
